...
from const import DEFAULT_PORT, SCAN_INTERVAL
...
_user_input_first = RequestUserInput(
    ...
    [
        ...
        },
        {
            ...
            "field": {"number": {"value": DEFAULT_PORT, "min": 1, "max": 65535, "steps": 1, "decimals": 0}},
        },
    ],
)
...
async def handle_configuration_mode(msg: UserDataResponse) -> RequestUserInput | SetupComplete | SetupError:
    ...
    match action:
        ...
        case "configure":
            ...
            return RequestUserInput(
                {
                    "en": "Configure your MPC-HC player",
                    "fr": "Configurez votre lecteur MPC-HC",
                },
                [
                    {
                    ...
                    },
                    {
                        ...
                        "label": {
                            "en": "Refresh frequency in seconds",
                            "fr": "Fréquence d'actualisation en secondes",
                        },
                        "field": {
                            "number": {
                                "value": _reconfigured_device.refresh_frequency,
                                "min": 5,
                                "max": 120,
                                "steps": 1,
                                "decimals": 0,
...
async def _handle_discovery(msg: UserDataResponse) -> RequestUserInput | SetupError:
    ...
    try:
        port = int(msg.input_values.get("port", str(DEFAULT_PORT)))
    except ValueError:
        port = DEFAULT_PORT
    ...
    try:
        device = MPCHCClient(
            DeviceInstance(
                id="MPC-HC", name="MPC-HC", address=address, port=port, refresh_frequency=10, always_on=False
            )
        )
        await device.connect()
        _LOG.debug("Looking for MPC-HC info...")
        friendly_name = await device.get_device_info()
        if friendly_name is None:
            _LOG.error("Cannot connect tos entered address %s:%s", address, port)
            return SetupError(error_type=IntegrationSetupError.CONNECTION_REFUSED)
        _LOG.debug("Found instance %s", friendly_name)
        dropdown_items.append({"id": address, "label": {"en": f"{friendly_name} [f{address}]"}})
    ...
    return RequestUserInput(
        {
            "en": "Please configure your MPC-HC player",
            "fr": "Configurez votre lecteur MPC-HC",
        },
        [
            {
...
async def handle_device_choice(msg: UserDataResponse) -> SetupComplete | SetupError:
    ...
    """
    # pylint: disable=W1405,W0718
    host = msg.input_values.get("address", "")
    try:
        port = int(msg.input_values.get("port", str(DEFAULT_PORT)))
        refresh_frequency = int(msg.input_values.get("refresh_frequency", str(SCAN_INTERVAL)))
    except ValueError:
        return SetupError(error_type=IntegrationSetupError.OTHER)
    always_on = msg.input_values.get("always_on") == "true"
    log_client = msg.input_values.get("log_client") == "true"
    _LOG.debug("Chosen MPC-HC: %s:%s. Trying to connect and check information...", host, port)
    try:
        ...
        _LOG.debug("Got data from device %s", info)
        if not info:
            _LOG.error("Could not get data from device %s:%s", host, port)
            return SetupError(error_type=IntegrationSetupError.CONNECTION_REFUSED)
    ...
    _LOG.info("Setup successfully completed for %s", unique_id)
    return SetupComplete()
...
async def _handle_device_reconfigure(msg: UserDataResponse) -> SetupComplete | SetupError:
    ...
    _reconfigured_device.address = host
    _reconfigured_device.port = port
    _reconfigured_device.refresh_frequency = refresh_frequency
