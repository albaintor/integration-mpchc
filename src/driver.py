...
def _configure_new_device(device_config: config.DeviceInstance, connect: bool = True) -> None:
    ...
    if device_config.id in _configured_devices:
        ...
        device = MPCHCClient(device_config)
...
def _register_available_entities(config_device: config.DeviceInstance, device: MPCHCClient) -> None:
    """
...
async def _async_remove(device: MPCHCClient) -> None:
...
async def main():
    ...
    for device in config.devices.all():
        _LOG.debug("UC MPC-HC device %s %s", device.id, device.address)
        _configure_new_device(device, connect=False)
