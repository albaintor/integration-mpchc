...
from client import Events, MPCHCClient
...
async def main():
    ...
    client = MPCHCClient(
        device_config=DeviceInstance(  # 192.168.1.129 192.168.1.132
            id="deviceid", name="MPCHC", address="192.168.1.24", port=DEFAULT_PORT, log_client=True
        )
    )
    client.events.on(Events.UPDATE, on_device_update)
    await client.connect()
    print_json(data=client._player_variables)
