...
class DeviceInstance:
    """MPC-HC device configuration."""

    id: str
    name: str
    address: str
    port: int = dataclasses.field(default=DEFAULT_PORT)
    always_on: bool = dataclasses.field(default=False)
    refresh_frequency: int = dataclasses.field(default=SCAN_INTERVAL)
    log_client: bool = dataclasses.field(default=False)
    ...
    def update(self, device_instance: DeviceInstance) -> bool:
        ...
        for item in self._config:
            if item.id == device_instance.id:
                ...
                item.refresh_frequency = device_instance.refresh_frequency
